...
import * as catalog from './danmaku-qq';
...
export async function supplementLocalMusicMetadata(
  ...
  if (supplement === undefined) {
    supplement = null;
    try {
      const res = await catalog.search(keywords, 0);
      ...
      if (matchedMusic && matchedScore >= 10) {
        ...
        try {
          const lyricRaw = await catalog.lyric(matchedMusic);
          const lyricText = typeof lyricRaw === 'string' ? lyricRaw : lyricRaw?.lyric || '';
