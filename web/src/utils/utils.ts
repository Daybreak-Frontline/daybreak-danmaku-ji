...
import { LyricCue, LyricLine, MusicFileInfo } from './type';
...
export function duration2Millisecond(duration: string) {
  ...
  const [clock, frac = ''] = duration.split('.');
  const date = (clock || '').split(':');
  const len = date.length;
  const digits = String(frac).replace(/\D/g, '');
  let millisecond = 0;
  if (digits) {
    // LRC: 1–2 digits are centiseconds; 3+ digits are milliseconds.
    millisecond = digits.length <= 2
      ? parseInt(digits.padEnd(2, '0'), 10) * 10
      : parseInt(digits.slice(0, 3), 10);
  }
  const second = parseInt(date[len - 1] || '0', 10) * 1000;
  const minute = parseInt(date[len - 2] || '0', 10) * 60 * 1000;
  const hour = parseInt(date[len - 3] || '0', 10) * 3600 * 1000;
  return (millisecond || 0) + (hour || 0) + (minute || 0) + (second || 0);
}
...
/** Catalog songs store duration in seconds; local/playback values may already be ms. */
export function musicLengthMs(music?: { length?: number; durationSec?: number } | null): number {
  const durationSec = Number(music?.durationSec);
  if (Number.isFinite(durationSec) && durationSec > 0) {
    return Math.round(durationSec * 1000);
  }
  const length = Number(music?.length);
  if (!Number.isFinite(length) || length <= 0) return 0;
  return length > 10000 ? Math.round(length) : Math.round(length * 1000);
}

export function parseLyric(text: string, length: number): LyricLine[] {
  if (!text || typeof text != 'string') return [];
  const durationMs = Number(length) > 0 ? Number(length) : 0;
  ...
  lines.forEach(line => {
    ...
    const text = decodeLyricText(line.replace(timeRegex, '').trim());
    let match;
    let matched = false;
  ...
  textList.forEach(line => {
    let progress = Math.min(
      Math.floor((1000 * line.millisecond) / durationMs),
      1000
    );
  ...
  for (let i = 0; i < lyricLines.length; i++) {
    ...
    if (!next) {
      ...
      lyricLines[i].endMs = durationMs;
    } else {
      lyricLines[i].max = next.progress;
      lyricLines[i].endMs = next.startMs ?? durationMs;
    }
  }
  return lyricLines;
}

export function parseSubtitleCues(cues: LyricCue[], length: number): LyricLine[] {
  const durationMs = Number(length) > 0 ? Number(length) : 0;
  if (!durationMs || !Array.isArray(cues) || !cues.length) return [];
  return cues.map((cue, index) => {
    const startMs = Math.max(0, Math.round(Number(cue.from) * 1000 || 0));
    const rawEnd = Number(cue.to);
    const nextStart = Number(cues[index + 1]?.from);
    const endMs = Number.isFinite(rawEnd) && rawEnd * 1000 > startMs
      ? Math.round(rawEnd * 1000)
      : Number.isFinite(nextStart) && nextStart * 1000 > startMs
        ? Math.round(nextStart * 1000)
        : durationMs;
    return {
      progress: Math.min(1000, Math.floor((1000 * startMs) / durationMs)),
      max: Math.min(1000, Math.floor((1000 * endMs) / durationMs) || 1000),
      text: cue.text,
      duration: '',
      startMs,
      endMs
